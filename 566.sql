...
WITH query_from_co_object AS (
    ...
    WHERE
        ...
        AND coep."KOKRS" = '1100'
        AND coep."LEDNR" = '00'
        -- CHANGE FROM 557: Date filter set to 2022 (vs 2020 in 557) for more recent data only
        AND cobk."BUDAT" >= '2022-01-01'

      AND (
    -- CHANGE FROM 557: Object numbers optimized using IN clauses instead of individual OR conditions
    -- This improves readability and query performance
    -- Cost center object numbers (KS prefix)
    coep."OBJNR" IN (
        'KS11000000060101', 'KS11000000060114', 'KS11000000060175', 'KS11000000060177', 
        'KS11000000060179', 'KS11000000060241', 'KS11000000060291', 'KS11000000060296', 
        'KS11000000060302', 'KS11000000060308', 'KS11000000060310', 'KS11000000060313', 
        'KS11000000060322', 'KS11000000060325', 'KS11000000060331', 'KS11000000060368', 
        'KS11000000060371', 'KS11000000060440', 'KS11000000060484', 'KS11000000060661', 
        'KS11000000060706', 'KS11000000060708', 'KS11000000060715', 'KS11000000060805', 
        'KS11000000060886', 'KS11000000060925', 'KS11000000060977'
    )
 ...
 OR coep."OBJNR" IN (
        'KS11000000062175', 'KS11000000062177', 'KS11000000062179', 'KS11000000062241', 
        'KS11000000062244', 'KS11000000062251', 'KS11000000062296', 'KS11000000062302', 
        'KS11000000062308', 'KS11000000062313', 'KS11000000062331', 'KS11000000062354', 
        'KS11000000062366', 'KS11000000062368', 'KS11000000062382', 'KS11000000062388', 
        'KS11000000062405', 'KS11000000062407', 'KS11000000062440', 'KS11000000062683', 
        'KS11000000062692', 'KS11000000062708', 'KS11000000062749', 'KS11000000062805', 
        'KS11000000062810', 'KS11000000062841', 'KS11000000062844', 'KS11000000064800'
    )
 ...
 OR coep."OBJNR" BETWEEN 'KS11000000064803' AND 'KS11000000064805'
    -- Activity type object numbers (KL prefix)
 ...
 OR coep."OBJNR" IN ('KL11000000060440814197', 'KL11000000060484814197')
 ...
 OR coep."OBJNR" BETWEEN 'KL11000000068741700102' AND 'KL11000000068741814310'
  ...
  AND 
      (
      -- CHANGE FROM 557: Explicit cost element list (13 specific values) instead of BETWEEN range
      -- 557 used: coep."KSTAR" BETWEEN '0030512001' AND '0081005603'
      -- This provides precise control over which cost elements are included
      coep."KSTAR" IN (
          '0039221100',
          '0081002130',
          '0032019000',
...
SELECT
    ...
    -- CHANGE FROM 557: Simplified MATERIAL and SERVICE calculations
    -- 557 had commented-out logic for additional cost elements with material checks
    -- This version uses clean, straightforward logic
    ROUND(SUM(CASE WHEN query_from_co_object.cost_element IN (
...
SELECT q.order_id,
       ...
       q.material_id,
-- CHANGE FROM 557: Enhanced NULL handling using COALESCE before TRIM
-- 557 used: TRIM(q.purchasing_document) <> '' which could fail on NULL values
-- This version is more robust for NULL safety
CASE 
  WHEN COALESCE(TRIM(q.purchasing_document), '') <> '' THEN lfa1."MCOD1"
  WHEN COALESCE(TRIM(q.material_id), '') <> '' THEN lfa2."MCOD1"
  ELSE NULL
END AS vendor_name,
    q.posting_month,  
    q.fiscal_year,
